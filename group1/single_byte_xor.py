def get_english_score(input_bytes):
    ...
    return sum([character_frequencies.get(chr(byte),0) for byte in input_bytes])
...
def get_best_score(ciphertext):
        potential_messages=[]
        for key_value in range(256):
            message=single_byte_xor(ciphertext,key_value)
...
def main():
    ...
    best_score=get_best_score(ciphertext)
